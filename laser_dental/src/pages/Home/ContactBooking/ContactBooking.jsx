    ...
    return (
      <div className="px-8 py-16 flex flex-col items-center text-center gap-4">
        ...
        <div className="mt-2 flex flex-col items-center gap-1.5 text-xs text-gray-400">
          <span>📍 {branches.find((b) => b.slug === submittedData.location)?.name} — {branches.find((b) => b.slug === submittedData.location)?.area}</span>
        ...
        <Field label="Branch" icon={MapPin} error={errors.location?.message}>
          <div className="relative">
            ...
            >
              ...
              {branches.map((b) => (
                <option key={b.slug} value={b.slug}>{b.name} — {b.area}, {b.city}</option>
              ))}
            </select>
            <ChevronDown size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
          </div>
...
// ── Clinic Hours card skeleton ──────────────────────────────────────────────
const ClinicHoursSkeleton = () => (
          ...
          <div className="bg-white rounded-2xl px-6 py-5 flex items-center gap-4 border border-gray-100"
            ...
            <div>
              <p className="text-xs text-gray-400 font-medium uppercase tracking-wider mb-0.5">
                {branchesLoading ? "Locations" : `${branches.length} Location${branches.length !== 1 ? "s" : ""}`}
              </p>
              <p className="text-sm font-bold text-gray-800">All in Dhaka</p>
              <p className="text-xs text-gray-400 mt-0.5">
                {branchesLoading ? "Loading..." : branches.map((b) => b.name).join(" & ")}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden"
              ...
              {!branchesLoading && <BookingForm branches={branches} />}
            </div>
          </div>
                  ...
                  return (
                    <div key={branch._id}>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs text-gray-400">{branch.area}, {branch.city}</span>
                      </div>
                      {(branch.hours || []).map((h, i) => (
                        <div key={i} className="flex items-center gap-2 ml-1 mb-1 text-sm text-gray-600">
